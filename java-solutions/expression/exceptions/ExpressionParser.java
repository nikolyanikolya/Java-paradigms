...
public class ExpressionParser implements TripleParser {
    ...
    private static final Map<String, Function<TemplateExpression, UnaryOperation>> UNARY_FUNCTION_LIST
            = Map.of("l0", LowerBitCount::new, "t0", UpperBitCount::new);

    private static final Set<Character> VARIABLES = Set.of('x', 'y', 'z');

    @Override
    public TemplateExpression parse(String expression) throws ParserException {
        return (new RunParser(new StringSource(expression))).parse();
    }

    private static class RunParser extends BaseParser {
        public RunParser(ExpressionSource source) {
            super(source);
        }

        public TemplateExpression parse() throws ParserException {
            var result = minMaxParse();
            expect(END);
            return result;
        }
        private TemplateExpression minMaxParse() throws ParserException {
        ...
        private TemplateExpression addSubParse() throws ParserException {
        ...
        private TemplateExpression mulDivParse() throws ParserException {
        ...
        private TemplateExpression parenParse() throws ParserException {
            skipWhitespace();
            if (test('(')) {
        ...
        private TemplateExpression numParse() throws ParserException {
            if (test('-')) {
                if (between('0', '9')) {
        ...
        private TemplateExpression variableParse() throws ParserException {
            char ch = getChar();
        ...
        private TemplateExpression bitFunctionsParse() {
